...
3. On the "Configure Project" screen:
   - **Root Directory**: click **Edit** next to it and set it to `apps/web`.
   - **Build & Output Settings** → override the **Build Command** with:
     ```
     cd ../.. && pnpm --filter @watchman/db generate && pnpm --filter @watchman/sdk build && pnpm --filter @watchman/web build
     ```
     Leave the Output Directory and Install Command at their defaults.
...
3. Railway creates one service from the repo root — click into it, then go to **Settings**:
   - **Build Command**:
     ```
   ...
   - **Start Command**:
     ...
     ```
   - Under **Deploy**, make sure this is a normal always-on service (Railway's default) — not a cron/scheduled job. It needs to run continuously.
...
- **Vercel build fails on the Prisma step** — double check the Root Directory is `apps/web` and the Build Command override was saved exactly as shown above.
