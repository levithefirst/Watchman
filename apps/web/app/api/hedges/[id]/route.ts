...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    const hedge = await db.hedge.findUnique({ where: { id }, include: { exposure: true, receipt: true, user: { select: { demo: true } } } });
    if (!hedge) return NextResponse.json({ error: "Hedge not found" }, { status: 404 });
    return NextResponse.json({ hedge: { ...hedge, demo: hedge.user.demo } });
  } catch (error) {
