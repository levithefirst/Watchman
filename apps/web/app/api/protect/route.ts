...
const validBody = (value: unknown): value is ProtectBody => {
  ...
  return (body.asset === "BTC" || body.asset === "ETH") &&
    typeof body.exposureUsd === "number" &&
    typeof body.protectionPct === "number" &&
    (body.windowSeconds === 900 || body.windowSeconds === 3600) &&
    typeof body.maxPremiumUsd === "number" &&
    (body.wallet === undefined || typeof body.wallet === "string") &&
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    if (!Number.isFinite(raw.exposureUsd) || raw.exposureUsd <= 0 || raw.exposureUsd > 10_000_000) {
      return NextResponse.json({ error: "Exposure must be between $0 and $10,000,000" }, { status: 400 });
    }
    ...
    const requestData: HedgeRequest = {
      exposureUsd: raw.exposureUsd,
      protectionPct: raw.protectionPct,
      maxPremiumUsd: raw.maxPremiumUsd,
    ...
    if (!isDemo && (!raw.wallet || !executionWallet || normaliseWallet(raw.wallet) !== executionWallet)) {
      return NextResponse.json({ error: "Wallet mode requires the connected wallet to match the configured execution signer." }, { status: 403 });
    }
    ...
    const user = await db.user.upsert({
      ...
      update: { demo: isDemo },
      create: { wallet: walletKey, demo: isDemo },
    });
    const exposure = await db.exposure.create({
      data: {
        userId: user.id,
        asset: raw.asset,
        amount: raw.exposureUsd,
        entryPrice: 1,
      },
    });
    const saved = await db.hedge.create({
      data: {
        userId: user.id,
        ...
        premiumUsd: filled * executionPrice,
        downPrice: executionPrice,
        txHash,
    ...
    return NextResponse.json({
      hedgeId: saved.id,
      txHash: txHash ?? null,
      simulated: !process.env.PRIVATE_KEY,
      quote: { ...quote, hedge: hedgeQuote, filled, executionPrice },
    });
  } catch (error) {
